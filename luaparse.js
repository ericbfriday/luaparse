...
(function (root, name, factory) {
  ...
  function scanNumericLiteral() {
    ...
    var literal = ('0' === character && (120 === next || 88 === next)) ?
  ...
  function readInt64Suffix() {
    ...
    var charCode = input.charCodeAt(index);
    if (117 === charCode || 85 === charCode) {
      ++index;
      var nextCode = input.charCodeAt(index);
      if (108 === nextCode || 76 === nextCode) {
        ++index;
        var nextNextCode = input.charCodeAt(index);
        if (108 === nextNextCode || 76 === nextNextCode) {
          ++index;
          return 'ULL';
        } else {
          // UL but no L
    ...
    } else if (108 === charCode || 76 === charCode) {
        ++index;
        var nextCode = input.charCodeAt(index);
        if (108 === nextCode || 76 === nextCode) {
          ++index;
  ...
  function readHexLiteral() {
    ...
    var charCode = input.charCodeAt(index);
    if (112 === charCode || 80 === charCode) {
      foundBinaryExponent = true;
      ++index;

      // Sign part is optional and defaults to 1 (positive).
      var signCode = input.charCodeAt(index);
      if (43 === signCode || 45 === signCode) {
        binarySign = (43 === signCode) ? 1 : -1;
        ++index;
      }
  ...
  function readDecLiteral() {
    ...
    if (101 === charCode || 69 === charCode) {
      ...
      var signCode = input.charCodeAt(index);
      if (43 === signCode || 45 === signCode) ++index;
