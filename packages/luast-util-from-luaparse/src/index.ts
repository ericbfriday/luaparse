import type { Root, LuastNode, Comment } from 'luast'
...
const SCALAR_FIELDS = new Set([
  'operator', 'name', 'indexer', 'raw', 'value', 'local',
])
...
function convertNode(node: LegacyNode): LuastNode {
  ...
  const luastType = TYPE_MAP[legacyType]
  if (luastType == null) {
  ...
  if (fields != null) {
    ...
    for (const field of fields) {
      ...
      const isArray = nodeArrayFields != null && nodeArrayFields.includes(field)

      if (isArray && Array.isArray(legacyValue)) {
        result[field] = legacyValue.map((child: LegacyNode) => convertNode(child))
  ...
  if (luastType === 'root' && Array.isArray(node['comments'])) {
    result['comments'] = (node['comments'] as LegacyNode[]).map(
      (c) => convertNode(c)
    ) as Comment[]
  }
...
function convertPosition(node: LegacyNode, result: Record<string, unknown>): void {
  ...
  const position: Record<string, unknown> = {
    start: {
      ...
      ...(range != null ? { offset: range[0] } : {}),
    },
    end: {
      line: loc.end.line,
      column: loc.end.column + 1,
      ...(range != null ? { offset: range[1] } : {}),
    },
  }
