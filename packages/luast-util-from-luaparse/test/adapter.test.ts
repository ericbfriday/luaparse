...
describe('fromLuaparse', () => {
  ...
  describe('statements', () => {
    ...
    it('converts IfStatement with clauses', () => {
      const tree = convert('if true then x = 1 elseif false then x = 2 else x = 3 end')
  ...
  describe('function declarations', () => {
    it('converts named function with local = false', () => {
      ...
      const fn = tree.body[0] as Record<string, unknown>
      expect(fn['type']).toBe('functionDeclaration')
      expect(fn['local']).toBe(false)
      expect(fn['identifier']).not.toBeNull()
      expect(fn['isLocal']).toBeUndefined()
    })

    it('converts local function with local = true', () => {
      const tree = convert('local function foo() end')
      const fn = tree.body[0] as Record<string, unknown>
      expect(fn['local']).toBe(true)
    })
    ...
    it('converts function with parameters', () => {
      ...
      const fn = tree.body[0] as Record<string, unknown>
      const params = fn['parameters'] as Array<Record<string, unknown>>
      expect(params).toHaveLength(3)
      expect(params[0]!['type']).toBe('identifier')
      expect(params[2]!['type']).toBe('varargLiteral')
    })
  })
  ...
  describe('expressions', () => {
    it('converts Identifier (strips isLocal)', () => {
      ...
      const retStmt = tree.body[1] as Record<string, unknown>
      const args = retStmt['arguments'] as Array<Record<string, unknown>>
      const ident = args[0]!
    ...
    it('converts BinaryExpression', () => {
      const tree = convert('return 1 + 2')
      const retStmt = tree.body[0] as Record<string, unknown>
      const args = retStmt['arguments'] as Array<Record<string, unknown>>
      expect(args[0]!['type']).toBe('binaryExpression')
    ...
    it('converts LogicalExpression', () => {
      ...
      const retStmt = tree.body[0] as Record<string, unknown>
      const args = retStmt['arguments'] as Array<Record<string, unknown>>
      expect(args[0]!['type']).toBe('logicalExpression')
      expect(args[0]!['operator']).toBe('and')
    })
    ...
    it('converts UnaryExpression', () => {
      ...
      const retStmt = tree.body[0] as Record<string, unknown>
      const args = retStmt['arguments'] as Array<Record<string, unknown>>
      expect(args[0]!['type']).toBe('unaryExpression')
    })
    ...
    it('converts IndexExpression', () => {
      ...
      const retStmt = tree.body[0] as Record<string, unknown>
      const args = retStmt['arguments'] as Array<Record<string, unknown>>
      expect(args[0]!['type']).toBe('indexExpression')
    })
    ...
    it('converts TableConstructorExpression to tableConstructor', () => {
      ...
      const retStmt = tree.body[0] as Record<string, unknown>
      const args = retStmt['arguments'] as Array<Record<string, unknown>>
      expect(args[0]!['type']).toBe('tableConstructor')
    })
  })
  ...
  describe('table fields', () => {
    it('converts TableKey', () => {
      ...
      const retStmt = tree.body[0] as Record<string, unknown>
      const args = retStmt['arguments'] as Array<Record<string, unknown>>
      const table = args[0]! as Record<string, unknown>
  ...
  describe('literals', () => {
    ...
    it('converts BooleanLiteral', () => {
      ...
      const retStmt = tree.body[0] as Record<string, unknown>
      const args = retStmt['arguments'] as Array<Record<string, unknown>>
      expect(args[0]!['type']).toBe('booleanLiteral')
    ...
    it('converts NilLiteral', () => {
      ...
      const retStmt = tree.body[0] as Record<string, unknown>
      const args = retStmt['arguments'] as Array<Record<string, unknown>>
      expect(args[0]!['type']).toBe('nilLiteral')
    ...
    it('converts VarargLiteral', () => {
      ...
      const fn = tree.body[0] as Record<string, unknown>
      const body = fn['body'] as Array<Record<string, unknown>>
      const retStmt = body[0]!
      const args = retStmt['arguments'] as Array<Record<string, unknown>>
      expect(args[0]!['type']).toBe('varargLiteral')
    })
  })
