...
;(function (root, definition) {
  ...
  var getClass = {}.toString,
          ...
          result +=
            '\\"\b\f\n\r\t'.indexOf(symbol) > -1
              ...
              : // If the character is a control character, append its Unicode escape
                ...
                symbol < ' '
                ? '\\u' + toPaddedString(4, symbol.charCodeAt(0).toString(16))
                : symbol
        }
        return result + '"'
      }
        ...
        switch (className) {
          ...
          case '[object Date]':
            ...
            return (
              '"' +
              (year <= 0 || year >= 1e4
                ? (year < 0 ? '-' : '+') + toPaddedString(6, Math.abs(year))
                : toPaddedString(4, year)) +
              '-' +
              toPaddedString(2, value.getUTCMonth() + 1) +
              '-' +
              toPaddedString(2, value.getUTCDate()) +
              // Months, dates, hours, minutes, and seconds should have two
              // digits; milliseconds should have three.
              'T' +
              toPaddedString(2, value.getUTCHours()) +
              ':' +
              toPaddedString(2, value.getUTCMinutes()) +
              ':' +
              toPaddedString(2, value.getUTCSeconds()) +
              // Milliseconds are optional in ES 5.0, but required in 5.1.
              '.' +
              toPaddedString(3, value.getUTCMilliseconds()) +
              'Z"'
            )
          case '[object RegExp]':
            return substitute(
              '{"source": %s, "global": %s, "ignoreCase": %s, "multiline": %s}',
              quote(value.source),
              value.global,
              value.ignoreCase,
              value.multiline
    ...
    serializeQuery = (exports.serializeQuery = function serializeQuery(
      map,
      separator
    ) {
      var result = [],
    ...
    substitute = (exports.substitute = function substitute(value) {
      ...
      for (
        result = '', argument = 1, index = 0, length = value.length;
        index < length;
        index += 1
      ) {
      ...
      function onEvent(event) {
        switch (event.type) {
          ...
          case 'teardown':
            print(
              substitute(
                'Finished test `%s`. %d assertions, %d failures.',
                event.target.name,
                event.target.assertions,
                event.target.failures
              )
            )
            break
          case 'complete':
            print(
              substitute(
                'Finished spec `%s`. %d assertions, %d failures.',
                this.name,
                this.assertions,
                this.failures
              )
    ...
    createTAP = (exports.createTAP = function createTAP(print) {
      ...
      function onEvent(event) {
        switch (event.type) {
          case 'assertion':
            print(
              substitute('ok %d %s', ++testNumber, description(event.message))
            )
            break
          case 'failure':
            print(
    ...
    buildNode = exports.buildNode = (function () {
      ...
      var supportsExtendedElementSyntax = (function () {
        ...
        serializeAttribute = function serializeAttribute(name, value) {
          textNode.data = '' + value
          return (
            name + '="' + textElement.innerHTML.replace(/"/g, '&quot;') + '"'
          )
        },
          ...
          if (hasProperties) {
            if (Spec.hasKey(properties, 'style')) {
              Spec.forOwn(properties.style, function (property, value) {
                ...
                element.style[
                  (1, /(?:style|css)Float/.test(property) ? cssFloat : property)
                ] = value
              })
      ...
      function onEvent(event) {
        ...
        switch (type) {
          ...
          case 'start':
            ...
            this.elements = {
              header: element.appendChild(
                buildNode('h1', {className: 'suiteHeader'}, [
                  document.createTextNode(this.name)
                ])
              ),
              // Representing the status of the suite (e.g., running, passed, failed).
              status: element.appendChild(
                buildNode('div', {className: 'suiteStatus suiteRunning'})
              ),
              // Contains the aggregate suite results.
              results: element.appendChild(
                buildNode('ol', {className: 'testList'})
              ),
              // Contains the aggregate suite summary.
              stats: element.appendChild(
                buildNode('p', {className: 'suiteSummary'}, [
          ...
          case 'setup':
            ...
            this.elements.results.appendChild(
              buildNode('li', {className: 'testBlock testRunning'}, [
                buildNode('strong', {onclick: onClick, className: 'testName'}, [
          ...
          case 'teardown':
            this.elements.results.childNodes[this.position].className =
              target.failures ? 'testBlock testFailed' : 'testBlock testPassed'
            break
          // Update the suite status and cumulative summary.
          case 'complete':
            // Set the spec status.
            this.elements.status.className = target.failures
              ? 'suiteStatus suiteFailed'
              : 'suiteStatus suitePassed'
            // Create the aggregate spec summary.
            clearElement(this.elements.stats).appendChild(
          ...
          case 'failure':
            ...
            if (
              !(messages =
                this.elements.results.childNodes[this.position].childNodes[1])
            ) {
              // Hide the messages by default.
              messages = buildNode('ol', {
                className: 'assertionList',
                style: {display: 'none'}
              })
              this.elements.results.childNodes[this.position].appendChild(
                messages
              )
            }
