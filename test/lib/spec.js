...
;(function (root, Spec) {
  ...
  var getClass = {}.toString,
    ...
    Environment = (exports.Environment = {
      ...
      java:
        ...
        java != null &&
        getClass.call(java) == '[object JavaPackage]' &&
        typeof JavaAdapter == 'function',
    ...
    hasKey = (exports.hasKey = (function () {
      var memo = {},
        hasOwnProperty = memo.hasOwnProperty,
        hasKey
      // The `__proto__` property can't be set more than once in Gecko.
      Environment.mutableProto =
        ((memo.__proto__ = null), (memo.__proto__ = {1: 2}), memo)[1] == 2
      memo = null

      if (
        (Environment.hasOwnProp =
          getClass.call(hasOwnProperty) == '[object Function]')
      ) {
        ...
        hasKey = function hasKey(object, property) {
          ...
          return (
            property in object &&
            !(property in parent && object[property] === parent[property])
          )
        }
      }
      return hasKey
        ...
        if (className == '[object Array]') {
          ...
          if (result) {
            ...
            while (size--) {
              ...
              if (
                !(result =
                  size in left == size in right &&
                  eq(left[size], right[size], stack))
              ) {
                break
              }
            }
  ...
  function addListener(event, callback, context) {
    if (event && callback) {
      forEach(
        event.split(' '),
        function (event) {
          var callbacks = hasKey(this.events, event) && this.events[event],
  ...
  function removeListener(event, callback, context) {
    if (!event) {
      ...
      forEach(
        event.split(' '),
        function (event) {
          var target = hasKey(this.events, event) && this.events[event],
            previous
          if (target) {
            ...
            if (callback) {
              ...
              for (
                ;
                (target = hasKey(target, 'next') && target.next) != previous;
              ) {
                if (
                  (hasKey(target, 'callback') && target.callback != callback) ||
                  (context &&
                    hasKey(target, 'context') &&
                    target.context != context)
                ) {
    ...
    if ((target = hasKey(this.events, type) && this.events[type])) {
      ...
      for (; (target = hasKey(target, 'next') && target.next) != previous; ) {
        ...
        try {
          call.call(
            target.callback,
            (hasKey(target, 'context') && target.context) || this,
            event
          )
        } catch (exception) {
          error = exception
    ...
    if (all) {
      ...
      for (; (all = hasKey(all, 'next') && all.next) != previous; ) {
        try {
          call.call(
            all.callback,
            (hasKey(all, 'context') && all.context) || this,
            event
          )
        } catch (exception) {
          error = exception
  ...
  function index(position) {
    ...
    for (; position < length; position += 1) {
      ...
      if (
        test &&
        typeof test.constructor == 'function' &&
      ...
      case 'teardown':
        ...
        if (
          (typeof expected == 'number' ||
            getClass.call(expected) == '[object Number]') &&
          expected != this.assertions
        ) {
          this.emit({
            type: 'failure',
  ...
  function assertError(callback, expected, message) {
    var ok = false,
      isFunction = typeof expected == 'function'
    ...
    try {
      ...
      ok =
        expected == null ||
        (isFunction && call.call(expected, this, exception, this))
    }
